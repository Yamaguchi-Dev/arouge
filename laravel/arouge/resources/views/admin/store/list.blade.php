...
function goCsv()
{
    document.form1.action = "{{route('store_csv_download')}}";
    document.form1.submit();
}
...
function view_conf(view_status, id) {
    ...
    if (result) {
        window.location.href = "{{route('store_view_change')}}/"+id;
    }
}
</script>
				<form name="form1" action="{{route('store_list_search')}}" method="post">
@csrf
					<div class="elem-form">
						<dl>
							<dt>店名</dt>
...
@endforeach
						...
						<dl>
							...
							<dd>
								<ul class="horizontal">
									<li><label><input type="checkbox" name="bland[]" value="1" @if(!empty($search->bland) && in_array(1, $search->bland)) checked @endif/><span class="label">アルージェ</span></label></li>
									<li><label><input type="checkbox" name="bland[]" value="2" @if(!empty($search->bland) && in_array(2, $search->bland)) checked @endif/><span class="label">エンリッチ</span></label></li>
								</ul>
							</dd>
						</dl>
						<div class="submit">
...
@foreach($data as $k => $v)
							<tr>
								...
								<td>@if($v->bland_arouge == 1)〇@else - @endif</td>
								<td>@if($v->bland_enrich == 1)〇@else - @endif</td>
		...
		(function (d, w) {
			...
			Array.from(d.querySelectorAll('.elem-list-result button[data-id]')).forEach(function (el) {
				el.addEventListener('click', function () {
					...
					iframe.src = '{{route('store_edit')}}/' + this.dataset.id
