...
use App\Models\User;
use App\Models\GeleerichStore;
use App\Models\UplodeDateGeleerich;
...
class StoreGeleerichController extends Controller
{
    ...
    public function list(SearchRequest $search_req)
    {
        ...
        $query = GeleerichStore::where('status', '=', 1);
        ...
        if (!empty($search->bland)) {
            $query->where(function($q2) use($search) {
                if (in_array(1, $search->bland)) {
                    $q2->orWhere('bland_lueur', '=', "1");
                }
                if (in_array(2, $search->bland)) {
                    $q2->orWhere('bland_geleerich', '=', "1");
                }
                if (in_array(3, $search->bland)) {
                    $q2->orWhere('bland_etin', '=', "1");
                }
            });
        }
    ...
    public function csv_download(SearchRequest $search_req)
    {
        ...
        $file_name = "geleerich_store_".date('YmdHis').".csv";
        $headers = [ //ヘッダー情報
    ...
    public function edit($id = null)
    {
        ...
        $data = array();
        if (!empty($id)) {
            $store_query = GeleerichStore::where("id", "=", $id);
        ...
        return view('admin.store_geleerich.edit', compact('data'));
    }
    ...
    public function update(StoreRequest $request)
    {
        ...
        $store->tel = $data['tel'];
        $store->bland_lueur = in_array(1, $data['bland']) ? 1 : 0;
        $store->bland_geleerich = in_array(2, $data['bland']) ? 1 : 0;
        $store->bland_etin = in_array(3, $data['bland']) ? 1 : 0;

        $store->save();

        return redirect(route('store_geleerich_edit_complete'));
    }

    public function edit_complete()
    {
        return view('admin.store_geleerich.edit_complete');
    }
    ...
    public function csv_input()
    {
        ...
        $update_date = "";
        $update_res = UplodeDateGeleerich::where("status", "=", 1)->orderBy('id', 'desc')->first();
        if (!empty($update_res)) {
            $update_date = date("Y.m.d", strtotime($update_res->created_at));
        }

        $title = "ジュレリッチ取扱店CSVアップロード";
        return view('admin.store_geleerich.csv_input', compact('errors', 'title', 'update_date'));
    }
    ...
    public function csv_upload(Request $request)
    {
        ...
        if (empty($request->file('file_csv'))) {
            ...
            return redirect()->route("store_geleerich_csv_input");
        }
        ...
        foreach ($data as $k => $v) {
            ...
            // お取り扱い状況
            if ($v[8] != 1 && $v[8] != 0) {
                $errors[] = $row.'行目のリュール取扱状況は0か1のどちらかを入力してください。';
            }
            if ($v[9] != 1 && $v[9] != 0) {
                $errors[] = $row.'行目のジュレリッチ取扱状況は0か1のどちらかを入力してください。';
            }
            if ($v[10] != 1 && $v[10] != 0) {
                $errors[] = $row.'行目のエタン取扱状況は0か1のどちらかを入力してください。';
            }
        }
        ...
        // 初期設定 全てのステータスを2にする
        GeleerichStore::where('status', '=', 1)->update(["status" => 2]);
        ...
        foreach ($data as $k => $v) {
            ...
            $stores->bland_lueur = $v[8];
            $stores->bland_geleerich = $v[9];
            $stores->bland_etin = $v[10];
        ...
        $update_date = new UplodeDateGeleerich();
    ...
    {
        if (empty($id)) return redirect(route('store_geleerich_list'));

        // データチェック
        $query = GeleerichStore::where('id', '=', $id);

        if ($query->count() < 1) {
            return redirect(route('store_geleerich_list'));
        }
